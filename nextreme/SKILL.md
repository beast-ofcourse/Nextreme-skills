...
Break the user's task into phases from this palette: **design** (`system-architect` / `nextreme-decision`), **direction** (`next-best-thing` / `next-big-thing`), **build** (the implementation skill or `next-big-thing`'s gated build), **test** (`nextreme-tdd`), **review** (`nextreme-review`), **diagnose** (`bug-diagnostician` when present), **safety** (`git-guardrails`). Drop phases that don't apply; keep the order honest.
